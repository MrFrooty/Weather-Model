...
for month in range(12):
    ...
    lin_reg.fit(years, rainfall[:, month])
    models[month] = lin_reg
    # multi_poly = PolynomialFeatures(degree=2)
    # years_poly = multi_poly.fit_transform(years_train)
    # #multi_poly.fit(years_poly, rainfall[:, month])
    # lin_reg_multi = LinearRegression()
    # lin_reg_multi.fit(years_poly, rainfall_train)
    # models[month] = lin_reg_multi
    # rainfall_preds = lin_reg_multi.predict(multi_poly.fit_transform(years_test))
